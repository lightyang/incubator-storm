...
(defn- mk-receive-queue-map [storm-conf executors]
  (->> executors
       ...
       (map (fn [e] [e (disruptor/disruptor-traced-queue storm-conf
                                                  (str "receive-queue" e)
    ...
    (fn this
      ...
      ([callback]
         (let [version (.assignment-version storm-cluster-state storm-id callback)
               ...
               temp-outbound-timeout (- (current-time-secs) (conf TOPOLOGY-TEMP-CONNECTION-TIMEOUT-SECS))
               _ (log-message "Removing timeout temp-outbound-tasks older than " temp-outbound-timeout
                              ", current: " @temp-outbound-task->start-time-secs)
               _ (swap! temp-outbound-task->start-time-secs
                        (partial filter-val #(> % temp-outbound-timeout)))
               _ (log-message "new temp-outbound-tasks: " @temp-outbound-task->start-time-secs)
